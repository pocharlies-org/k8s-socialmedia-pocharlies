...
function tool(args: Omit<SocialToolDefinition, 'outputSchema' | 'annotations'> & {
  idempotent?: boolean;
  destructive?: boolean;
  openWorld?: boolean;
...
export const SOCIAL_TOOL_REGISTRY: readonly SocialToolDefinition[] = [
  tool({
    ...
    description: 'List configured provider accounts, transports and capabilities actually deployed.',
    effect: 'read',
  ...
  tool({
    ...
    description: 'List conversations from one channel/account or aggregate explicitly reported partial results.',
    effect: 'read',
    authScope: 'social.read',
  ...
  tool({
    ...
    inputSchema: objectSchema({ ...commonProperties, readSource }, ['channel', 'accountId', 'target']),
    idempotent: true,
  }),
  tool({
    name: 'social_list_participants',
    title: 'List participants',
    description: 'List participants of a group or conversation when supported by the selected provider.',
    effect: 'read',
    authScope: 'social.read',
  ...
  tool({
    ...
    description: 'Search indexed or provider messages without inferring a channel from target shape.',
    effect: 'read',
    authScope: 'social.read',
  ...
  tool({
    ...
    description: 'Resolve a name, username or address to provider-native targets usable by send operations.',
    effect: 'read',
    authScope: 'social.read',
  ...
  tool({
    ...
    inputSchema: objectSchema({
      ...commonProperties,
      range: { type: 'string', enum: ['conversation', 'context', 'day', 'week'], default: 'conversation' },
  ...
  tool({
    ...
    inputSchema: objectSchema({ channel, accountId, target, readSource }, ['channel', 'accountId', 'target']),
    idempotent: true,
  }),
  tool({
  ...
  tool({
    ...
    inputSchema: objectSchema({
      ...
      accountId,
      query: { type: 'string', minLength: 1 },
      kind: { type: 'string', enum: ['business', 'hashtag', 'hashtagMedia'], default: 'business' },
      limit: { type: 'integer', minimum: 1, maximum: 100, default: 25 },
      readSource,
    }, ['channel', 'accountId', 'query']),
    idempotent: true,
  ...
  tool({
    ...
    inputSchema: objectSchema({
      ...
      template: {
        type: 'object',
        description: 'Provider template payload; unsupported on deployed Baileys WhatsApp accounts.',
      },
    }, ['channel', 'accountId', 'target'], {
      anyOf: [
        { required: ['message'] },
  ...
  tool({
    ...
    description: 'Create, edit, close, pin or delete a Telegram topic, or manage group membership and permissions.',
    effect: 'destructive',
    authScope: 'social.write',
  ...
  tool({
    ...
    inputSchema: objectSchema({
      ...writeProperties,
      action: { type: 'string', enum: ['setTitle', 'setDescription', 'setPhoto', 'setForumEnabled'] },
      title: { type: 'string', minLength: 1 },
      description: { type: 'string' },
